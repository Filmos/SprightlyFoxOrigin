...
function applySideEffects(server, selector, stats, dist) {
  ...
  if(severity > 1) server.runCommandSilent(`/effect give ${selector} minecraft:blindness ${Math.round(3*(severity-1))}`)
  if(severity > 2) server.runCommandSilent(`/effect give ${selector} minecraft:hunger ${2+Math.round(4*severity)} ${Math.floor(1+severity)}`)
